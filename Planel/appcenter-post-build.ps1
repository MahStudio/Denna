...
$preRelease = $TRUE
$releaseNotes=$megtxt
$artifactOutputDirectory="$env:APPCENTER_SOURCE_DIRECTORY\Planel\Planel\AppPackages"
...
$gitHubApiKey="$env:GithubSicktear"
    ...
    $releaseData = @{
       tag_name = [string]::Format("V1.1.{0}", $versionNumber);
       name = [string]::Format("V1.1.{0}", $versionNumber);
       body = $releaseNotes;
       draft = $draft;
